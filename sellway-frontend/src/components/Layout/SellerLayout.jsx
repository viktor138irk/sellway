    ...
    <div style={{ display: 'flex', minHeight: 'calc(100vh - 90px)' }}>
      ...
      <aside style={{ width: 220, background: '#0F0F18', borderRight: `1px solid ${C.border}`, flexShrink: 0, display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '20px 16px', borderBottom: `1px solid ${C.border}`, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 58, height: 58, borderRadius: 14, background: `linear-gradient(135deg,${C.accent},#A78BFA)`,
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 11, color: C.t2, marginTop: 2 }}>Продавец</div>
          </div>
        ...
        <nav style={{ flex: 1, padding: '10px 8px', overflowY: 'auto' }}>
          {NAV.map(([to, icon, label]) => {
            ...
            return (
              <Link key={to} to={to} style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '9px 10px', borderRadius: 8,
                marginBottom: 2, background: active ? C.accent + '18' : 'transparent',
                borderLeft: `3px solid ${active ? C.accent : 'transparent'}`,
                color: active ? C.accent : C.t2, fontSize: 13, fontWeight: active ? 700 : 400,
