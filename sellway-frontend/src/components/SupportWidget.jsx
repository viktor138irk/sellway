...
export default function SupportWidget() {
  ...
  return <>
    {open && <section className="support-chat-panel" style={{ position:'fixed', right:20, bottom:82, width:340, height:450, maxWidth:'calc(100vw - 24px)', maxHeight:'calc(100vh - 108px)', zIndex:5100, background:C.card, border:`1px solid ${C.border}`, borderRadius:14, boxShadow:'0 16px 48px rgba(0,0,0,.55)', overflow:'hidden', display:'flex', flexDirection:'column' }}>
      ...
      {!user ? <div style={{ flex:1, padding:22, display:'flex', flexDirection:'column', justifyContent:'center', textAlign:'center', gap:14 }}>
        ...
        <div style={{ flex:1, padding:12, overflowY:'auto', display:'flex', flexDirection:'column', gap:9 }}>
          ...
          {messages.map(m => <div key={m.id} style={{ display:'flex', justifyContent:m.sender_type === 'user' ? 'flex-end' : 'flex-start' }}>
            <div style={{ maxWidth:'85%', background:m.sender_type === 'user' ? C.accent : '#1A1A28', borderRadius:m.sender_type === 'user' ? '12px 12px 4px 12px' : '12px 12px 12px 4px', padding:'9px 11px', color:'#fff', fontSize:13, lineHeight:1.45, whiteSpace:'pre-wrap', wordBreak:'break-word' }}>{m.message}</div>
          </div>)}
          <div ref={bottomRef} />
        </div>
