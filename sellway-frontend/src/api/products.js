import client from './client';
export const getProducts  = (params) => client.get('/products', { params });
export const getProduct   = (id) => client.get(`/products/${id}`);
export const getFeaturedOverview = () => client.get('/products/featured/overview');
export const createProduct = (data) => client.post('/products', data);
export const updateProduct = (id, data) => client.put(`/products/${id}`, data);
export const deleteProduct = (id) => client.delete(`/products/${id}`);
export const uploadImages  = (id, formData) => client.post(`/products/${id}/images`, formData, { headers: { 'Content-Type': 'multipart/form-data' } });
export const uploadProductFile = (id, formData) => client.post(`/products/${id}/file`, formData, { headers: { 'Content-Type': 'multipart/form-data' } });
export const addKeys       = (id, keys) => client.post(`/products/${id}/keys`, { keys });
export const getKeys       = (id) => client.get(`/products/${id}/keys`);
export const deleteKey     = (id, keyId) => client.delete(`/products/${id}/keys/${keyId}`);
export const getCategories = (params) => client.get('/categories', { params });
export const createCategory = (data) => client.post('/categories', data);
export const updateCategory = (id, data) => client.put(`/categories/${id}`, data);
export const deleteCategory = (id) => client.delete(`/categories/${id}`);
export const bulkImportCategories = (data) => client.post('/categories/bulk-import', data);
export const bulkDeleteCategories = (ids) => client.post('/categories/bulk-delete', { ids });
