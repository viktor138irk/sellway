...
function ProductCard({ p }) {
  ...
  return (
    <div onClick={() => navigate(`/product/${p.id}`)} onMouseEnter={() => setHov(true)} onMouseLeave={() => { setHov(false); setImgIdx(0); }} style={{ background: C.card, border: `1px solid ${hov ? C.accent+'55' : C.border}`, borderRadius: 14, overflow: 'hidden', cursor: 'pointer', transition: 'all .2s', display: 'flex', flexDirection: 'column' }}>
      <div style={{ position: 'relative', aspectRatio: '1', background: '#0A0A14', overflow: 'hidden' }}>
        {images.length > 0 ? <img src={images[imgIdx]} alt={p.title} style={{ width:'100%', height:'100%', objectFit:'cover', transition:'transform .3s', transform: hov?'scale(1.04)':'scale(1)' }} /> : <div style={{ width:'100%', height:'100%', display:'flex', alignItems:'center', justifyContent:'center', fontSize:52 }}>{service ? '🧑‍💻' : '📦'}</div>}
      ...
      <div style={{ padding:'12px 13px', display:'flex', flexDirection:'column', gap:6, flex:1 }}>
        <div style={{ display:'flex', alignItems:'baseline', gap:7 }}><span style={{ fontSize:18, fontWeight:900, color:C.t1 }}>{service ? 'от ' : ''}{parseFloat(p.price).toLocaleString('ru')} ₽</span>{p.old_price && <span style={{ fontSize:12, color:C.t3, textDecoration:'line-through' }}>{parseFloat(p.old_price).toLocaleString('ru')} ₽</span>}</div>
        ...
        <button onClick={e=>e.stopPropagation()} style={{ background:hov?C.accent:'#1A1A28', border:`1px solid ${hov?'transparent':C.border}`, color:'#fff', borderRadius:8, padding:'9px', fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:'inherit', transition:'all .2s', marginTop:2 }}>{service ? 'Обсудить' : hov ? 'Купить сейчас →' : 'Подробнее'}</button>
      </div>
    </div>
  );
}
...
export default function HomePage() {
  ...
  return <div style={{ maxWidth:1200, margin:'0 auto', padding:'28px 20px', display:'flex', flexDirection:'column', gap:48, width:'100%', boxSizing:'border-box' }} className="fade-in">
    ...
    {rootCats.length > 0 && <section>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:18 }}><h2 style={{ fontSize:20, fontWeight:900, color:C.t1 }}>Категории</h2><Link to="/catalog" style={{ fontSize:13, color:C.accent, textDecoration:'none' }}>Все →</Link></div>
      ...
      {selectedRoot && <div style={{ marginTop:14, background:C.card, border:`1px solid ${C.border}`, borderRadius:16, padding:16 }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', gap:10, marginBottom:12, flexWrap:'wrap' }}><div style={{ fontSize:14, fontWeight:900, color:C.t1 }}>Подкатегории: {selectedRoot.name}</div><button type="button" onClick={() => navigate(`/catalog?category=${selectedRoot.slug}`)} style={{ background:'transparent', border:`1px solid ${C.border}`, color:C.accent, borderRadius:8, padding:'7px 12px', fontSize:12, cursor:'pointer', fontFamily:'inherit' }}>Открыть всю категорию</button></div>
        {children.length > 0 ? <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(130px,1fr))', gap:10 }}>{children.map(c => <CategoryCard key={c.id} cat={c} onClick={() => navigate(`/catalog?category=${c.slug}`)} />)}</div> : <div style={{ color:C.t3, fontSize:13 }}>У этой категории пока нет подкатегорий.</div>}
      </div>}
    </section>}

    {popular.length > 0 && <section><div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:18 }}><h2 style={{ fontSize:20, fontWeight:900, color:C.t1 }}>🔥 Популярное</h2><Link to="/catalog?sort=popular" style={{ fontSize:13, color:C.accent, textDecoration:'none' }}>Смотреть все →</Link></div><div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(220px,1fr))', gap:16 }}>{popular.map(p=><ProductCard key={p.id} p={p}/>)}</div></section>}
    {newest.length > 0 && <section><div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:18 }}><h2 style={{ fontSize:20, fontWeight:900, color:C.t1 }}>✨ Новинки</h2><Link to="/catalog?sort=newest" style={{ fontSize:13, color:C.accent, textDecoration:'none' }}>Смотреть все →</Link></div><div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(220px,1fr))', gap:16 }}>{newest.map(p=><ProductCard key={p.id} p={p}/>)}</div></section>}
