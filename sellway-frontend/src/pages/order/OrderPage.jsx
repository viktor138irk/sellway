...
export default function OrderPage() {
  ...
  return <div style={{ maxWidth:780, margin:'0 auto', padding:'28px 20px' }} className="fade-in">
    ...
    <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:18, overflow:'hidden' }}>
      ...
      {!isBuyer && order.status === 'paid' && <div style={{ padding:'16px 20px', background:'#10101F', borderTop:`1px solid ${C.border}` }}><div style={{ fontSize:13, color:C.t2, marginBottom:12 }}>После передачи покупатель сможет проверить заказ, оценить вас и подтвердить оплату.</div><Btn full variant="green" loading={actionLoading==='deliver'} onClick={handleDeliver}>Передать заказ покупателю</Btn></div>}
