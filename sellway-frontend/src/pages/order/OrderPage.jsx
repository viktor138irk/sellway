  ...
  return <div style={{ maxWidth:780, margin:'0 auto', padding:'28px 20px' }} className="fade-in">
    ...
    <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:18, overflow:'hidden' }}>
      <div style={{ background:'#0A0A12', padding:'16px 22px', borderBottom:`1px solid ${C.border}` }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16, gap:10, flexWrap:'wrap' }}><div><div style={{ fontSize:13, fontWeight:800, color:C.t1 }}>Сделка {order.order_number}</div><div style={{ fontSize:11, color:C.t3, marginTop:2 }}>{new Date(order.created_at).toLocaleString('ru')}</div></div><div style={{ display:'flex', alignItems:'center', gap:10 }}><StatusBadge status={service && order.status === 'delivered' ? 'service_delivered' : order.status}/><span style={{ fontSize:12, background:'#0A2A1A', color:C.green, padding:'4px 12px', borderRadius:20, fontWeight:700 }}>Escrow</span></div></div>
      ...
      {!isBuyer && order.status === 'paid' && <div style={{ padding:'16px 20px', background:'#10101F', borderBottom:`1px solid ${C.border}` }}>
      ...
      {isBuyer && order.status === 'delivered' && <div style={{ padding:'16px 20px', background:'#0A1E10', borderBottom:`1px solid #1A4020` }}>
      ...
      {isBuyer && order.auto_delivery_message && ['delivered','confirmed'].includes(order.status) && <div style={{ padding:'14px 22px', borderBottom:`1px solid ${C.border}`, background:'#10101F' }}><div style={{ fontSize:12, color:C.accent, fontWeight:800, marginBottom:8 }}>Инструкция продавца</div><div style={{ color:C.t1, fontSize:13, lineHeight:1.6, whiteSpace:'pre-wrap' }}>{order.auto_delivery_message}</div></div>}
      ...
      {order.status === 'confirmed' && <div style={{ padding:20, background:'#0A1E10', borderTop:`1px solid #1A4020`, textAlign:'center' }}><div style={{ fontSize:16, fontWeight:800, color:C.green, marginBottom:6 }}>Сделка завершена</div><div style={{ fontSize:13, color:C.t2 }}>{isBuyer ? (service ? 'Выполнение услуги подтверждено.' : 'Спасибо за покупку.') : 'Средства зачислены на ваш баланс.'}</div></div>}
      {!isBuyer && order.status === 'confirmed' && !order.buyer_review && <div style={{ padding:18, background:'#10101F', borderTop:`1px solid ${C.border}` }}><div style={{ fontSize:14, fontWeight:900, color:C.t1, marginBottom:12 }}>{service ? 'Оцените заказчика' : 'Оцените покупателя'}</div><div style={{ display:'grid', gap:12 }}><RatingPicker value={buyerRating} onChange={setBuyerRating} label={service ? 'Рейтинг заказчика' : 'Рейтинг покупателя'} /><Textarea rows={3} value={buyerReview} onChange={e=>setBuyerReview(e.target.value)} placeholder="Комментарий необязателен" /><Btn loading={actionLoading==='rate-buyer'} onClick={handleRateBuyer}>Сохранить оценку</Btn></div></div>}
      {!isBuyer && order.buyer_review && <div style={{ padding:16, background:'#10101F', borderTop:`1px solid ${C.border}` }}><div style={{ fontSize:12, color:C.t2, marginBottom:5 }}>Ваша оценка {service ? 'заказчика' : 'покупателя'}</div><Stars n={order.buyer_review.rating} size={14}/>{order.buyer_review.comment && <div style={{ marginTop:6, color:C.t2, fontSize:13 }}>{order.buyer_review.comment}</div>}</div>}
