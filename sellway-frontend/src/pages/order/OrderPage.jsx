...
import { getOrder, sendMessage, confirmOrder, openDispute } from '../../api/orders';
import { sendServiceProposal, acceptServiceProposal } from '../../api/serviceOrders';
import { C, Spinner, Btn, StatusBadge, Modal, Input, Textarea } from '../../components/UI';
...
export default function OrderPage() {
  ...
  async function handleConfirm() { setActionLoading('confirm'); try { await confirmOrder(id); toast.success('Получение подтверждено'); loadOrder(); } catch (err) { toast.error(err.response?.data?.error || 'Ошибка'); } finally { setActionLoading(''); } }
  ...
  return <div style={{ maxWidth:780, margin:'0 auto', padding:'28px 20px' }} className="fade-in">
    ...
    <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:18, overflow:'hidden' }}>
      ...
      <div style={{ padding:'14px 22px', borderBottom:`1px solid ${C.border}`, display:'flex', justifyContent:'space-between', alignItems:'center', gap:12, flexWrap:'wrap' }}><div><div style={{ fontSize:14, fontWeight:700, color:C.t1 }}>{order.product_title}</div><div style={{ fontSize:12, color:C.t2, marginTop:2 }}>{isBuyer ? `${service?'Фрилансер':'Продавец'}: ${order.seller_name}` : `Покупатель: ${order.buyer_name}`}</div></div><div style={{ textAlign:'right' }}><div style={{ fontSize:18, fontWeight:900, color:C.t1 }}>{service && order.meta?.negotiation_status === 'requested' ? 'от ' : ''}{money(order.amount)}</div>{!isBuyer && <div style={{ fontSize:11, color:C.green }}>Вы получите: {money(order.seller_amount)}</div>}</div></div>
      {service && <ProposalPanel order={order} isBuyer={isBuyer} onReload={loadOrder}/>} 
      {isBuyer && order.key_value && ['delivered','confirmed'].includes(order.status) && <div style={{ padding:'14px 22px', borderBottom:`1px solid ${C.border}`, background:'#0A1A0A' }}><div style={{ fontSize:12, color:C.green, fontWeight:700, marginBottom:8 }}>Ваш товар/ключ</div>{showKey ? <div style={{ background:'#111', borderRadius:8, padding:'12px 16px', fontFamily:'monospace', fontSize:14, color:C.t1, letterSpacing:1, wordBreak:'break-all' }}>{order.key_value}</div> : <Btn size="sm" variant="green" onClick={()=>setShowKey(true)}>Показать ключ</Btn>}</div>}
      ...
      {isBuyer && order.status === 'delivered' && <div style={{ padding:'16px 20px', background:'#0A1E10', borderTop:`1px solid #1A4020` }}><div style={{ fontSize:13, color:C.green, fontWeight:700, marginBottom:12 }}>Позиция передана. Проверьте и подтвердите получение.</div><div style={{ display:'flex', gap:10, flexWrap:'wrap' }}><Btn full variant="green" loading={actionLoading==='confirm'} onClick={handleConfirm}>Подтвердить получение</Btn><Btn full variant="danger" onClick={()=>setDisputeModal(true)}>Открыть спор</Btn></div></div>}
