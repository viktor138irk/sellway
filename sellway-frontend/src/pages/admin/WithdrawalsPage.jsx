    ...
    <AdminLayout>
      <div style={{ padding:'24px 28px' }} className="fade-in">
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:22 }}>
          <h1 style={{ fontSize:28, fontWeight:650, color:C.t1 }}>Выплаты</h1>
        ...
        : list.length === 0 ? <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:8, padding:60, textAlign:'center', color:C.t3 }}><div style={{ fontFamily:'var(--sw-serif)', color:C.accent, fontSize:24, marginBottom:12 }}>Payouts</div><div style={{ color:C.t2, fontSize:14 }}>Заявок нет</div></div>
        : <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:8, overflow:'hidden' }}>
            <div style={{ display:'grid', gridTemplateColumns:'180px 1fr 120px 120px 100px 100px', gap:12, padding:'10px 18px', background:C.field, borderBottom:`1px solid ${C.border}` }}>
                ...
                <div key={w.id} style={{ display:'grid', gridTemplateColumns:'180px 1fr 120px 120px 100px 100px', gap:12, padding:'13px 18px', borderBottom:`1px solid ${C.border}`, alignItems:'center', transition:'background .15s' }}
                  ...
                  {filter === 'pending' && (
                    <div style={{ display:'flex', gap:5 }}>
                      <button onClick={()=>handleApprove(w.id)} disabled={actionLoading===w.id} style={{ background:C.green, border:'none', color:'#fff', borderRadius:7, padding:'5px 9px', fontSize:11, cursor:'pointer', fontWeight:700 }}>Одобрить</button>
                      <button onClick={()=>{ setRejectModal(w.id); setRejectReason(''); }} style={{ background:'transparent', border:`1px solid ${C.red}`, color:C.red, borderRadius:7, padding:'5px 9px', fontSize:11, cursor:'pointer' }}>Нет</button>
                    </div>
                  )}
                </div>
              );
