  ...
  return (
    <Modal title={`Спор по заказу ${dispute.order_number}`} onClose={onClose} width={580}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          {[['Покупатель', dispute.buyer_name, 'buyer'], ['Продавец', dispute.seller_name, 'seller']].map(([role, name, val]) => (
            <div key={val} onClick={() => setWinner(val)}
              style={{ background: winner === val ? (val==='buyer'?C.green:C.accent)+'18' : C.field,
                border: `2px solid ${winner===val ? (val==='buyer'?C.green:C.accent) : C.border}`,
                borderRadius: 8, padding: '14px 16px', cursor: 'pointer', transition: 'all .15s', textAlign: 'center' }}>
        ...
        {/* Order info */}
        <div style={{ background: C.field, borderRadius: 8, padding: 14, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, fontSize: 12 }}>
        ...
        {/* Reason */}
        <div style={{ background: `${C.amber}12`, border: `1px solid ${C.amber}33`, borderRadius: 8, padding: '12px 16px' }}>
        ...
        <div>
          ...
          <textarea value={resolution} onChange={e => setResolution(e.target.value)} rows={3}
            ...
            style={{ width: '100%', background: C.field, border: `1px solid ${C.border}`, borderRadius: 8, padding: '10px 13px', color: C.t1, fontSize: 13, outline: 'none', fontFamily: 'inherit', resize: 'vertical' }} />
        </div>

        {winner && (
          <div style={{ background: (winner==='buyer'?C.green:C.accent)+'15', border: `1px solid ${(winner==='buyer'?C.green:C.accent)}33`, borderRadius: 8, padding: '12px 16px', fontSize: 12, color: winner==='buyer'?C.green:C.accent }}>
            {winner === 'buyer'
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <Btn full disabled={!winner} loading={loading} onClick={handleResolve}>Вынести решение</Btn>
        </div>
      </div>
    </Modal>
  );
    ...
    <AdminLayout>
      <div style={{ padding: '24px 28px' }} className="fade-in">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 22, flexWrap: 'wrap', gap: 12 }}>
          <h1 style={{ fontSize: 28, fontWeight: 650, color: C.t1 }}>Споры / Арбитраж</h1>
        ...
        {loading
          ...
          : disputes.length === 0
            ? <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 8, padding: 60, textAlign: 'center' }}>
                <div style={{ fontFamily:'var(--sw-serif)', color:C.accent, fontSize: 24, marginBottom: 12 }}>Arbitrage</div>
                <div style={{ fontSize: 15, color: C.t2 }}>Споров нет</div>
              </div>
            : <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {disputes.map(d => (
                  <div key={d.id} style={{ background: C.card, border: `1px solid ${d.status==='open'?C.red+'44':C.border}`, borderRadius: 8, padding: '18px 20px' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr auto auto auto', gap: 14, alignItems: 'flex-start', marginBottom: 12 }}>
                      ...
                      {filter === 'open' && <Btn size="sm" onClick={() => setSelected(d)}>Рассмотреть</Btn>}
                    ...
                    <div style={{ background: `${C.amber}12`, borderRadius: 8, padding: '10px 14px', fontSize: 12, color: C.t2, lineHeight: 1.5 }}>
                      <span style={{ color: C.amber, fontWeight: 700 }}>Причина: </span>{d.reason}
                    </div>
                    {d.resolution && (
                      <div style={{ background: `${C.green}12`, borderRadius: 8, padding: '10px 14px', fontSize: 12, color: C.green, marginTop: 8 }}>
