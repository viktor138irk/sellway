    ...
    const [wallet, seller, recentOrders, products, referralStats, currentUser] = await Promise.all([
      ...
      query(`SELECT s.*,
             ...
             LEFT JOIN LATERAL (
               ...
               WHERE delivery.seller_id = s.user_id
...
const WITHDRAW_METHODS = { card: { label: 'Банковская карта', icon: 'CARD', placeholder: 'Номер карты' }, sbp: { label: 'СБП (Быстрые платежи)', icon: 'SBP', placeholder: 'Номер телефона' }, paypal: { label: 'PayPal', icon: 'PP', placeholder: 'Email PayPal' }, crypto: { label: 'USDT', icon: 'USDT', placeholder: 'Адрес кошелька USDT' } };
...
async function getWithdrawConfig(client = { query }, userId = null) {
  const { rows } = await client.query(
    `SELECT key, value FROM settings WHERE key IN (
      'min_withdrawal','max_withdrawal_daily','usdt_rub_rate_fallback','withdrawal_commission',
  ...
  let personalWithdrawalRate = null;
  let seller = null;
  if (userId) {
    const result = await client.query(
      `SELECT auto_payout_enabled, auto_payout_method, auto_payout_threshold, auto_payout_requisites,
              custom_withdrawal_commission_rate
       FROM sellers WHERE user_id=$1`,
      [userId]
    );
    seller = result.rows[0];
    personalWithdrawalRate = seller?.custom_withdrawal_commission_rate != null
      ? Number(seller.custom_withdrawal_commission_rate)
      : null;
  }
  const defaultWithdrawalRate = Number(settings.withdrawal_commission ?? 0.02);
  const methods = Object.entries(WITHDRAW_METHODS).map(([id, meta]) => ({
    id,
    ...meta,
    enabled: settings[`withdraw_method_${id}_enabled`] !== 'false',
    commission: id === 'crypto'
      ? 0
      : (personalWithdrawalRate ?? Number(settings[`withdraw_method_${id}_commission`] ?? defaultWithdrawalRate)),
    commissionSource: id === 'crypto' ? 'crypto_free' : personalWithdrawalRate != null ? 'personal' : 'method',
  }));
  const config = {
  ...
  if (userId) {
    ...
    config.personalWithdrawalCommissionRate = personalWithdrawalRate;
  }
  return config;
}
...
router.get('/commercial-status', async (req, res) => {
  try {
    const { rows: [seller] } = await query('SELECT * FROM sellers WHERE user_id=$1', [req.user.id]);
    const { rows: [defaultCommission] } = await query("SELECT value FROM settings WHERE key='default_seller_commission_rate' LIMIT 1");
    const saleCommissionRate = seller?.custom_commission_rate != null
      ? Number(seller.custom_commission_rate)
      : Number(defaultCommission?.value ?? process.env.PLATFORM_COMMISSION ?? 0.07);
    const access = describeCommercialAccess(req.user, seller);
    res.json({
      access,
      seller: seller || null,
      role: req.user.role,
      saleCommissionRate,
      saleCommissionSource: seller?.custom_commission_rate != null ? 'personal' : 'default',
      requirements: {
...
router.put('/withdrawal/auto-payout', requireApprovedCommercial, async (req, res) => {
  ...
  try {
    ...
    if (enabled && method === 'crypto' && !USDT_NETWORKS.has(String(requisites.network || ''))) {
      return res.status(400).json({ error: 'Выберите сеть USDT' });
    }
