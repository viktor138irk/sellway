...
async function getCommissionRate(client, sellerId) {
  const { rows: [seller] } = await client.query(
    'SELECT custom_commission_rate FROM sellers WHERE user_id=$1',
    [sellerId]
  );
...
router.post('/', auth, async (req, res) => {
  ...
  try {
    await transaction(async (client) => {
      const { rows: [product] } = await client.query(
        `SELECT p.*, u.id AS seller_user_id
         ...
         WHERE p.id=$1 AND p.status='active' AND p.delivery_type='service'`,
        [product_id]
      );
      ...
      const meta = {
        service: true,
        price_mode: 'from',
        start_price: Number(product.price),
      ...
      const { rows: [order] } = await client.query(
        `INSERT INTO orders (buyer_id, seller_id, product_id, status, amount, commission, seller_amount, delivery_type, meta)
         VALUES ($1,$2,$3,'pending',$4,0,0,'service',$5::jsonb)
         RETURNING *`,
        [req.user.id, product.seller_user_id, product_id, product.price, JSON.stringify(meta)]
      );

      await client.query(
        `INSERT INTO order_messages (order_id, sender_id, message, is_system)
         VALUES ($1,$2,$3,TRUE)`,
        [order.id, req.user.id, '🧩 Заявка на услугу создана. Цена пока предварительная — от ' + Number(product.price).toLocaleString('ru') + ' ₽.']
      );
      ...
      if (message?.trim()) {
        await client.query(
          `INSERT INTO order_messages (order_id, sender_id, message)
           VALUES ($1,$2,$3)`,
          [order.id, req.user.id, message.trim()]
        );
      }
    ...
    logger.error('Create service order error', { err: err.message });
...
router.post('/:id/proposal', auth, requireRole('freelancer', 'admin'), async (req, res) => {
  ...
  try {
    await transaction(async (client) => {
      const { rows: [order] } = await client.query(
        "SELECT * FROM orders WHERE id=$1 AND delivery_type='service' FOR UPDATE",
        [req.params.id]
      );
      ...
      if (!['pending', 'paid'].includes(order.status)) throw { status: 400, message: 'Смету можно отправить только в открытой сделке' };
      if (order.status === 'paid') throw { status: 400, message: 'Смета уже утверждена заказчиком' };
      ...
      const { rows: [updated] } = await client.query(
        `UPDATE orders SET amount=$1, commission=$2, seller_amount=$3, meta=$4::jsonb
         WHERE id=$5 RETURNING *`,
        [amount, commission, sellerAmount, JSON.stringify(meta), order.id]
      );

      await client.query(
        `INSERT INTO order_messages (order_id, sender_id, message, is_system)
         VALUES ($1,$2,$3,TRUE)`,
        [order.id, req.user.id, '💼 Фрилансер отправил смету на ' + amount.toLocaleString('ru') + ' ₽. Заказчик должен подтвердить стоимость перед резервированием средств.']
      );
    ...
    if (err.status) return res.status(err.status).json({ error: err.message });
    logger.error('Service proposal error', { err: err.message });
    return res.status(500).json({ error: 'Ошибка сервера' });
  }
});
...
router.post('/:id/accept-proposal', auth, async (req, res) => {
  try {
    await transaction(async (client) => {
      const { rows: [order] } = await client.query(
        "SELECT * FROM orders WHERE id=$1 AND buyer_id=$2 AND delivery_type='service' AND status='pending' FOR UPDATE",
      ...
      if (!proposal || meta.negotiation_status !== 'awaiting_customer') {
        throw { status: 400, message: 'Нет сметы для подтверждения' };
      }
      ...
      await client.query(
        ...
        [order.buyer_id, order.id, amount, 'Резерв по услуге ' + order.order_number, JSON.stringify({ source: 'service_proposal' })]
      );

      const { rows: [updated] } = await client.query(
        "UPDATE orders SET status='paid', paid_at=NOW(), amount=$1, commission=$2, seller_amount=$3, meta=$4::jsonb WHERE id=$5 RETURNING *",
        [amount, Number(proposal.commission || 0), Number(proposal.seller_amount || 0), JSON.stringify(meta), order.id]
      );

      await client.query(
        `INSERT INTO order_messages (order_id, sender_id, message, is_system)
         VALUES ($1,$2,$3,TRUE)`,
        [order.id, order.buyer_id, '✅ Заказчик подтвердил смету. Средства зарезервированы на платформе.']
      );
    ...
    if (err.status) return res.status(err.status).json({ error: err.message });
    logger.error('Accept service proposal error', { err: err.message });
    return res.status(500).json({ error: 'Ошибка сервера' });
  }
});
